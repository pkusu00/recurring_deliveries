...
def dag_jira():
    ...
    retrieve_and_clean_jira_task = PythonOperator(
        task_id = "retrieve_and_clean_jira_all",
        python_callable = retrieve_and_clean_jira_file
    )

    retrieve_and_transform_jira_history_task = PythonOperator(
        task_id = "retrieve_and_transform_jira_history",
        python_callable = retrieve_and_transform_jira_history
    )
    ...
    send_local_jira_file_to_gcs_task = LocalFilesystemToGCSOperator(
        ...
        dst= f"jira-all/all_jira_cleaned_{current_date}.csv",
        bucket= "echo_jira"
    )
